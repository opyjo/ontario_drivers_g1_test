...
import { studyGuideData, type StudyGuideChapter, type StudyGuideSection } from "@/data/study-guide"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { ChevronRight, Clock, BookOpen, ChevronLeft, Menu, FileText, ChevronDown, ChevronUp } from "lucide-react"

export default function StudyGuidePage() {
  const [selectedChapter, setSelectedChapter] = useState<StudyGuideChapter | null>(null)
  ...
  const navigateToNextSection = () => {
    ...
    const currentSectionIndex = selectedChapter.sections.findIndex((sec) => sec.id === selectedSection.id)
    const currentChapterIndex = studyGuideData.findIndex((ch) => ch.id === selectedChapter.id)
    ...
    <div className="w-80 bg-white border-r border-gray-200 h-full overflow-y-auto">
      <div className="p-6 border-b border-gray-200">
        ...
        <p className="text-sm text-gray-600 mt-1">
          {studyGuideData.reduce((acc, ch) => acc + ch.sections.length, 0)} sections available
        </p>
      </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500 mt-1">{chapter.sections.length} sections</div>
                    </div>
                  </div>
                </button>
              </div>
  ...
  if (selectedChapter && selectedSection) {
    const currentSectionIndex = selectedChapter.sections.findIndex((sec) => sec.id === selectedSection.id)
    const currentChapterIndex = studyGuideData.findIndex((ch) => ch.id === selectedChapter.id)
    const isFirstSection = currentChapterIndex === 0 && currentSectionIndex === 0
    const isLastSection =
      currentChapterIndex === studyGuideData.length - 1 && currentSectionIndex === selectedChapter.sections.length - 1
          ...
          <div className="max-w-4xl mx-auto px-4 py-8">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <button onClick={() => setSelectedChapter(null)} className="hover:text-cyan-600 transition-colors">
                  Study Guide
                </button>
                <ChevronRight className="w-4 h-4" />
                <button onClick={() => setSelectedSection(null)} className="hover:text-cyan-600 transition-colors">
            ...
            <Card className="mb-6">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-2xl text-gray-900">{selectedSection.title}</CardTitle>
                </div>
              </CardHeader>
                ...
                <div className="prose max-w-none">
                  <div className="bg-blue-50 border-l-4 border-blue-400 p-6 rounded-r-lg">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-4">{renderHTMLContent(selectedSection.content)}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-6">
                  ...
                  <div className="space-y-3">
                    {selectedSection.keyPoints.map((point, index) => {
                      if (point.includes("\n") || point.includes("•")) {
                        const [mainPoint, ...subItems] = point.split(/\n|•/).filter((item) => item.trim())
                        return (
                          <div key={index} className="bg-gray-50 rounded-lg p-4 hover:bg-gray-100 transition-colors">
                            <div className="flex items-start gap-4">
                              <div className="flex items-center justify-center w-6 h-6 bg-cyan-100 text-cyan-700 rounded-full text-sm font-semibold flex-shrink-0 mt-0.5">
                                {index + 1}
                              </div>
                ...
                <div className="bg-gradient-to-r from-orange-50 to-yellow-50 border border-orange-200 rounded-lg p-6">
                  ...
                  <div className="space-y-2">
                    <p className="text-orange-700 leading-relaxed">
                      Take your time to understand each key point. Consider how these rules apply to real driving
                      situations you might encounter on Ontario roads.
                    </p>
                    <div className="bg-orange-100 rounded-lg p-3 mt-3">
                      <p className="text-orange-800 text-sm font-medium">
                        📚 Pro Tip: Review this section multiple times and try to explain the concepts in your own words
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Button className="bg-cyan-600 hover:bg-cyan-700">Practice Questions</Button>
              </div>
        ...
        <div className="flex-1 overflow-hidden">
          <div className="max-w-4xl mx-auto px-4 py-8">
              ...
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <CardDescription className="text-lg mb-3">{selectedChapter.description}</CardDescription>
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      <p className="text-sm text-blue-800">
                        📖 Content based on the Official Ministry of Transportation (MTO) Driver's Handbook
                      </p>
                    </div>
                  </div>
                      ...
                      <div className="border-t border-gray-200 bg-gray-50">
                        <div className="p-6 space-y-6">
                          <div className="bg-blue-50 border-l-4 border-blue-400 p-6 rounded-r-lg">
                            <div className="flex items-start gap-3">
                              <FileText className="w-5 h-5 text-blue-600 mt-0.5 flex-shrink-0" />
                              <div className="space-y-4">{renderHTMLContent(section.content)}</div>
                            </div>
                          </div>
            ...
            <div className="flex justify-between mt-8">
              ...
              <Button className="bg-orange-600 hover:bg-orange-700">Take Chapter Quiz</Button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Ontario Driver's Study Guide</h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-4">
            Master the official MTO driver's handbook with our comprehensive study guide. Learn at your own pace with
            structured chapters and practice questions.
          </p>
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-2xl mx-auto">
            <p className="text-sm text-blue-800">
              📖 All content is based on the Official Ministry of Transportation (MTO) Driver's Handbook for Ontario
            </p>
          </div>
        </div>
          ...
          {studyGuideData.map((chapter) => (
            ...
            >
              <CardHeader>
                ...
                <CardTitle className="text-xl text-gray-900">{chapter.title}</CardTitle>
