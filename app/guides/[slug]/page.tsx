...
export default async function GuideArticlePage({ params }: Props) {
  ...
  const relatedGuides = getRelatedGuideArticles(article);
  return (
    <PageLayout>
      ...
      <article className="container mx-auto max-w-4xl px-4 py-14">
        ...
        <header>
          <p className="text-sm font-semibold uppercase tracking-wider text-primary">{article.category} guide</p>
        ...
        <section className="mt-10 rounded-xl border bg-blue-50/60 dark:bg-blue-950/40 p-6"><h2 className="text-xl font-bold">Key takeaways</h2><ul className="mt-4 space-y-3">{article.takeaways.map((item) => <li key={item} className="flex gap-3 leading-7"><CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-primary" aria-hidden="true" />{item}</li>)}</ul></section>
        ...
        <div className="mt-14 border-t pt-12">
          <GuideRecommendations
            ...
            title="Related G1 guides"
            description="Continue with the next topic that supports this part of your preparation."
          />
        </div>
      </article>
    </PageLayout>
